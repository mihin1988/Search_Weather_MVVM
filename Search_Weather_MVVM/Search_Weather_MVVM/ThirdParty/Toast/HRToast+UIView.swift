...
//  VendorApp
//
//  Created by Mihin  Patel on 07/05/20.
...
public struct HRToastConfig {
    var HRToastDefaultDuration  =   5.0
...
public extension UIView {
    ...
    fileprivate func makeToastActivity(position pos: AnyObject, message msg: String = "", withConfiguration config: HRToastConfig) {
        ...
        let activityConatinView = UIView(frame: CGRect(x: 0, y: 0, width: SceneDelegate.shared?.window?.frame.size.width ?? 320, height: SceneDelegate.shared?.window?.frame.size.height ?? 480))
        activityConatinView.backgroundColor = UIColor(red: 0/255, green: 0/255, blue: 0/255, alpha: 0.6)
...
//        activityIndicatorView.color = Constants.themeColor
        ...
        activityConatinView.addSubview(activityView)
        addSubview(activityConatinView)
        
        // associate activity view with self
        objc_setAssociatedObject(self, &HRToastActivityView, activityConatinView, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
